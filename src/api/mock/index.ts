...
// In-memory storage for newly created bookings (persists during app session)
const runtimeBookings: Booking[] = [];
...
export const mockAPI = {
  ...
  async getSalons(): Promise<Salon[]> {
    await delay(800);
    return salonsData as unknown as Salon[];
  },
  ...
  async getBookingsByCustomerId(customerId: string): Promise<Booking[]> {
    await delay(600);
    const allBookings = [...bookingsData, ...runtimeBookings];
    return allBookings.filter((b) => b.customerId === customerId) as Booking[];
  },

  async getBookingsBySalonId(salonId: string): Promise<Booking[]> {
    await delay(600);
    const allBookings = [...bookingsData, ...runtimeBookings];
    return allBookings.filter((b) => b.salonId === salonId) as Booking[];
  },
  ...
  async updateBookingStatus(
    ...
    // Update in runtime storage if it exists there
    const runtimeIndex = runtimeBookings.findIndex((b) => b.id === bookingId);
    if (runtimeIndex !== -1) {
      runtimeBookings[runtimeIndex] = {
        ...runtimeBookings[runtimeIndex],
        status,
      };
      return runtimeBookings[runtimeIndex];
    }
  ...
  async updateSalon(salonId: string, data: Partial<Salon>): Promise<Salon> {
    ...
    if (!salon) throw new Error("Salon not found");
    return { ...salon, ...data } as unknown as Salon;
  },
  ...
  async getOwnerStats(ownerId: string): Promise<{
    ...
    return {
      ...
      monthBookings: 156,
      averageRating:
        ownerSalons.reduce((sum, s) => sum + s.rating, 0) / ownerSalons.length,
    };
  },
};
